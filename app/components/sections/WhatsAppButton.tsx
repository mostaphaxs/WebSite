...
export default function WhatsAppInteraction() {
  ...
  useEffect(() => {
    // Hide the whole thing until the PageReveal animation is done (5.5s + buffer)
    const visibilityTimer = setTimeout(() => setIsVisible(true), 6500);

    // Auto-show the toast shortly after becoming visible
    const toastTimer = setTimeout(() => setShowToast(true), 8000);
        ...
        <>
          ...
          <AnimatePresence>
            {showToast && (
              ...
              >
                <div className="relative p-10 bg-white/95 backdrop-blur-3xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.1)] border border-zinc-100 group pointer-events-auto overflow-hidden rounded-[40px]">
